...
### Log valid referers:
if (isset($url) && (strstr(getenv("HTTP_REFERER"), $url))) {
  addRefer($url);
}
...
while ($story = mysql_fetch_object($result)) {
  ...
  $theme->abstract2($story->aid, $story->informant, $story->time, stripslashes($story->subject), stripslashes($story->abstract), stripslashes($story->comments), $story->category, $story->department, $morelink);
}
